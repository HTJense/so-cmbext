...
:Synopsis: Adaptation of MFLike (SO Multifrequency Likelihood) for the purposes of CMB extraction.
...
class SOCMBExt(InstallableLikelihood):
    ...
    def extract_cmb_spectra(self, **params_values_nocosmo):
        from scipy import linalg
        
        ps_vec = self._get_power_spectra(None, **params_values_nocosmo)
        
        ws = self.spec_meta[0]["bpw"].weight.shape[-1]
        
        # bb(:) = matmul(fisher, bb)
        bb = self.inv_cov @ (self.data_vec - ps_vec)
        mat = np.zeros(( self.cov.shape[0], ws + ws + ws ))
        
        ct = np.ones((len(self.spec_meta),))
        
        idx0 = { "tt" : 0, "te" : ws, "ee" : ws + ws }
        
        for i, spec in enumerate(self.spec_meta):
            nu1 = spec["nu1"]
            nu2 = spec["nu2"]
            pl1 = spec["pol"][0].upper()
            pl2 = spec["pol"][1].upper()
            
            pr1 = "cal" + pl1 + "_" + str(nu1)
            pr2 = "cal" + pl2 + "_" + str(nu2)
            
            ct[i] = 1.0 * params_values_nocosmo[pr1] * params_values_nocosmo[pr2]
        
        for i, spec in enumerate(self.spec_meta):
            for j, k in enumerate(spec["ids"]):
                mat[k,j + idx0[spec["pol"]]] = ct[i]
        
        # wbb_vec(:) = matmul(mat_tran, bb)
        wbb = mat.T @ bb
        
        # dum2 = matmul(fisher, mat)
        # dum = matmul(mat_tran, dum2)
        # dpotrf(..., dum, ...)
        # dpotri(..., dum, ...)
        dum = linalg.inv(mat.T @ self.inv_cov @ mat)
        
        # gauss_vec(:) = gauss()
        gn = np.random.normal(size = mat.shape[1])
        # dpotrf(..., dum, ...)
        ldum = linalg.cholesky(dum, lower = True)
        
        # wbb_vec = matmul(dum, wbb_vec)
        # dgemv(..., dum, gauss_vec, step, ...)
        # wbb_vec(:) += step(:)
        res = dum @ wbb + ldum @ gn
        
        return { "tt" : res[:ws], "te" : res[ws:ws+ws], "ee" : res[ws+ws:] }
    
    def _get_power_spectra(self, cl, **params_values_nocosmo):
        # Get Cl's from the theory code
        Dls = { s : np.zeros(self.l_bpws.max() + 1)[self.l_bpws] for s, _ in self.lcuts.items() }
